...
[...document.querySelectorAll('.js_modInvalidatecach')].forEach((button) => {
  button.addEventListener('click', async (event) => {
    let resp;
    const el = event.currentTarget;
    el.setAttribute('disabled', '');
    const url = new URL(`${el.dataset.url}administrator/index.php?option=com_ajax&format=json&module=publicfolder&method=create&${el.dataset.token}=1`);
    if (!url) return;
